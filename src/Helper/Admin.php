...
class Admin
{
    ...
    private static function add_hook($hook, $function, $priority = 10)
    {
        if ( ! self::is_hook_added($function)) {
            add_filter($hook, [__CLASS__, $function], $priority);
    ...
    private static function get_post_types($type)
    {
        switch (strtolower($type)) {
            case 'custom':
                return get_post_types(['_builtin' => false]);
            case 'builtin':
                return get_post_types(['_builtin' => true]);
            case 'all':
    ...
    public static function remove_template_metabox($metabox, $template)
    {
        ...
        if ( ! isset(self::$metaboxes_to_remove[$metabox])) {
            self::$metaboxes_to_remove[$metabox] = [];
        }
    ...
    {
        if (empty(self::$metaboxes_to_remove)) {
            return;
        }
    ...
    public static function remove_submenu_page($menu, $submenu)
    {
        ...
        $submenu = is_array($submenu) ? $submenu : [$submenu];
        if ( ! isset(self::$submenus_to_remove[$menu])) {
            self::$submenus_to_remove[$menu] = [];
        }
    ...
    public static function remove_post_metabox($id, $post_type, $context)
    {
        ...
        if (in_array($lower_post_type, ['builtin', 'custom', 'all'])) {
            // Keyword is used
            self::$post_metaboxes_to_remove[] = ['id' => $id, 'type' => $lower_post_type, 'context' => $context];
            ...
            foreach ($post_type as $type) {
                self::$post_metaboxes_to_remove[] = ['id' => $id, 'type' => $type, 'context' => $context];
            }
        }
    ...
    {
        if (empty(self::$post_metaboxes_to_remove)) {
            return;
        }
    ...
    public static function _set_admin_menu()
    {
        ...
        foreach (self::$separators_to_add as $index => $position) {
            $separator = ['', 'read', "separator-$position", '', 'wp-menu-separator'];
