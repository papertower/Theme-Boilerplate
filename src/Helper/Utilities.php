...
class Utilities {
   ...
   */
	public static function get_function_output($function, $arguments = array()) {
		$contents = '';
		ob_start();
  ...
  public static function get_the_template_part($slug, $name) {
    return self::get_function_output('get_template_part', array($slug, $name));
  }
  ...
  public static function include_directory($directory_path, $recursive = false, $once = true) {
    ...
    if ( !is_dir($directory_path) ) throw new Exception("$directory_path is not a valid directory");
    ...
    if ( false === $contents ) throw new Exception("error loading contents for $directory_path");

    // Parse through files and include
    foreach($contents as $file) {
      if ( is_dir($file) ) {
        // include directory files if recursive
        if ( $recursive ) self::include_directory("$file/", $recursive, $once);
      } else {
